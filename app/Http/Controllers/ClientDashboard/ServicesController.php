...
class ServicesController extends Controller {
    ...
    protected function manageservices() {
        $id = Auth::user()->id;
        $service_list = Model\Contents::where('admin_id', $id)->get();
    ...
    protected function saveservice(Request $request) {
        ...
        $id = Auth::user()->id;
        $service_task = Model\Contents::where('admin_id', $id)->first();
        if ($request->has('id'))
            $service_task = Model\Contents::where('admin_id', $id)->where('id', $request->id)->first();
        else if (empty($service_task)) {
            $service_task = new Model\Contents;
            $service_task->admin_id = $id;
        }
        $service_task->services = $request->service_content;
        $service_task->save();
        return back();
    }
